...
def registrar():
    if request.method == "POST":
        ...
        if ya_esta_adentro(documento):
            return mostrar_mensaje_exito("❌ Ya registró ingreso y no ha registradosu salida salida.")
...
function validarYMostrar() {
    ...
    // 👇 Aquí se corrigió \\s en la expresión regular
   ...
   if (!/^\d+$/.test(documento)) {
    ...
    if (!/^\d{10}$/.test(celular)) {
        mostrarError("El número de teléfono debe tener exactamente 10 dígitos numéricos.");
        return;
    }
...
def ya_esta_adentro(documento):
    ...
    for row in ws.iter_rows(min_row=7, values_only=True):  # Asumes que los datos inician en fila 7
        doc = str(row[5]).strip()  # DOCUMENTO DE IDENTIFICACIÓN
...
def salida():
    if request.method == "POST":
        ...
        if encontrada:
            wb.save(ARCHIVO_EXCEL)
            return mostrar_mensaje_exito("Ya registró ingreso y no ha registrado salida.")
